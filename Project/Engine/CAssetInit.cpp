...
void CAssetMgr::CreateEngineMaterial()
{
	...
	AddAsset(L"FinalBloomMtrl", pMtrl);

	// ThreshholdMtrl
	pMtrl = new CMaterial(true);
	pMtrl->SetShader(FindAsset<CGraphicShader>(L"ThreshholdShader"));
	AddAsset(L"ThreshholdMtrl", pMtrl);
}
...
void CAssetMgr::CreateEngineTexture()
{
	...
	Ptr<CTexture> pThresholdTex = CreateTexture(
		L"ThresholdTex"
		, (UINT)(vResolution.x), (UINT)(vResolution.y)
		, DXGI_FORMAT_R32G32B32A32_FLOAT, D3D11_BIND_RENDER_TARGET | D3D11_BIND_SHADER_RESOURCE);


	Ptr<CTexture> pBloomCopy = CreateTexture(L"BloomCopyTex"
		, (UINT)vResolution.x, (UINT)vResolution.y
		, DXGI_FORMAT_R8G8B8A8_UNORM, D3D11_BIND_SHADER_RESOURCE);
...
void CAssetMgr::CreateEngineGraphicShader()
{
	...
	pShader->SetBSType(BS_TYPE::ALPHABLEND);
	pShader->SetDomain(SHADER_DOMAIN::DOMAIN_POSTPROCESS);
	AddAsset(L"ThreshholdShader", pShader);

	// FinalBloomShader
	pShader = new CGraphicShader;
	pShader->CreateVertexShader(L"shader\\postprocess.fx", "VS_FinalBloom");
	pShader->CreatePixelShader(L"shader\\postprocess.fx", "PS_FinalBloom");
	pShader->SetRSType(RS_TYPE::CULL_NONE);
	pShader->SetDSType(DS_TYPE::NO_TEST_NO_WRITE);
	pShader->SetBSType(BS_TYPE::DEFAULT);
	pShader->SetDomain(SHADER_DOMAIN::DOMAIN_POSTPROCESS);
	AddAsset(L"FinalBloomShader", pShader);
}
