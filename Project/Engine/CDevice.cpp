...
int CDevice::Init(HWND _hWnd, UINT _Width, UINT _Height)
{
	...
	{
		MessageBox(nullptr, L"Rasterizer State 생성 실패", L"장치 초기화 실패", MB_OK);
		return E_FAIL;
	}
...
int CDevice::CreateDepthStencilState()
{
	...
	//Desc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ZERO;				// 깊이 판정을 성공시 깊이 기록 여부
	//Desc.StencilEnable  = true;
	//
	//// Stencil
	//Desc.BackFace.StencilFunc			= D3D11_COMPARISON_ALWAYS;	// 항상 통과
	//Desc.BackFace.StencilDepthFailOp	= D3D11_STENCIL_OP_KEEP;	// 깊이가 실패(뒷면 보다 더 멀다)
	//Desc.BackFace.StencilFailOp			= D3D11_STENCIL_OP_KEEP;	// 스텐실 테스트는 항상 통과기 때문에 발생할 일이 없다.
	//Desc.BackFace.StencilPassOp			= D3D11_STENCIL_OP_INCR;	// Stencil 에 값을 1 증가 시킨다.
	//
	...
	//// Stencil
	//
	//Desc.FrontFace.StencilFunc			= D3D11_COMPARISON_EQUAL;		// 특정 값(1) 이랑 동일 할 경우 Stencil 통과
	//Desc.FrontFace.StencilDepthFailOp	= D3D11_STENCIL_OP_KEEP;		// 스텐실 실패 == Inner 체크 성공을 못한 부분들, 아무것도 할 필요가 없다.
	//Desc.FrontFace.StencilFailOp	    = D3D11_STENCIL_OP_DECR;		// 스텐실 성공, 깊이 실패, 뒷면 보다는 안쪽에 있었지만 앞면보다도 더 앞쪽에 있었다.
	//Desc.FrontFace.StencilPassOp	    = D3D11_STENCIL_OP_KEEP;		// 스텐실 성공, 깊이 성공, 뒷면보다 안쪽에, 앞면보다 뒷쪽에, 볼륨메쉬 내부 영역
	//
	...
	if (FAILED(DEVICE->CreateDepthStencilState(&Desc, m_DSState[(UINT)DS_TYPE::VOLUME].GetAddressOf())))
	{
	    return E_FAIL;
	}
