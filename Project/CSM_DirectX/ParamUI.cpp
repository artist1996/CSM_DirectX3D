...
bool ParamUI::InputTexture(Ptr<CTexture>& _CurTex, const string& _Desc, EditorUI* _Inst, DELEGATE_1 _MemFunc)
{
	...
	if (ImGui::BeginDragDropTarget())
	{
		const ImGuiPayload* Payload = ImGui::AcceptDragDropPayload("FILE_BROWSER_ITEM");
		if (Payload)
		{
			const wchar_t* szItem = (const wchar_t*)Payload->Data;

			Ptr<CTexture> pAsset = CAssetMgr::GetInst()->FindAsset<CTexture>(szItem);
